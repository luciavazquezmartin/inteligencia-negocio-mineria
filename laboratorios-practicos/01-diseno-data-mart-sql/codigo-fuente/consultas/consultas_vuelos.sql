...
-- 9. Análisis multidimensional de vuelos por hora y festividad
SELECT 
    h.franja_horaria,
    f.es_festivo,
    COUNT(*) as total_vuelos,
...
JOIN FECHA f ON v.id_fecha = f.id_fecha
GROUP BY CUBE(h.franja_horaria, f.es_festivo)
ORDER BY h.franja_horaria NULLS LAST, f.es_festivo NULLS LAST;

-- 10. Modelos con más de 1 vuelo y velocidad superior a 850 km/h
SELECT 
    av.fabricante,
    av.nombre_modelo,
    av.velocidad_maxima,
